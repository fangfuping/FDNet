...
logname = os.path.join(log_dir, '11'+'.txt')
...
with torch.no_grad():
    ...
    for ii, data_val in enumerate((val_loader), 0):
        ...
        with torch.cuda.amp.autocast():
            restored,signal,jam = model_restoration(input_)
...
for epoch in range(start_epoch, opt.nepoch + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        with torch.cuda.amp.autocast():
            restored,signal,jam = model_restoration(input_)
            loss1 = abs(coss(signal,jam))
            # loss1 = 0
            # lamda = 0.25
            # if opt.regulation == 'l1':
            #     loss1 = lamda*torch.norm(restored,p=1)/512/512 #/torch.max(torch.abs(restored))
            # if opt.regulation == 'l2':
            #     loss1 = lamda*torch.norm(restored,p=2)/512/512 #/torch.max(torch.abs(restored))
            # if opt.regulation == 'tvl1':
            #     dx = torch.mean(torch.abs(restored[:, :, :, :-1] - restored[:, :, :, 1:]))
            #     dy = torch.mean(torch.abs(restored[:, :, :-1, :] - restored[:, :, 1:, :]))
            #     loss1 = (dx+dy)*lamda
            # if opt.regulation == 'tvl2':
            #     dx1 = torch.mean(torch.pow(restored[:, :, :, :-1] - restored[:, :, :, 1:],2))
            #     dy1 = torch.mean(torch.pow(restored[:, :, :-1, :] - restored[:, :, 1:, :],2))
            #     loss1 = (dx1 + dy1) * lamda
            loss = criterion(restored, target)
            loss = loss + 0.00001*loss1
        loss_scaler(
