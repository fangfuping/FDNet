...
class UNet(nn.Module):
    def __init__(self, block=ConvBlock,dim=32):
        ...
        # self.noise = block(dim * 8, dim * 16, strides=1)
    ...
    def forward(self, x):
        ...
        # jam = self.noise(pool4)
        # signal = conv5.view(1,-1)
        # jam = jam.view(1,-1)
...
class FDUformer(nn.Module):
    def __init__(self, img_size=256, in_chans=1, dd_in=1,
        ...
        self.noise = BasicUformerLayer(dim=embed_dim * 16,
                                      output_dim=embed_dim * 16,
                                      input_resolution=(img_size // (2 ** 4),
                                                        img_size // (2 ** 4)),
                                      depth=depths[4],
    ...
    def forward(self, x, mask=None):
        ...
        jam = self.noise(pool3,mask = mask)
        signal = conv4.view(1,-1)
        jam  = jam.view(1,-1)
...
if __name__ == "__main__":
    ...
    xx = torch.randn((2, 1, 512, 512))
    y = model_restoration(xx)
