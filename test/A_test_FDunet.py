...
import utils
from utils.utils_image_2 import  tensor2uint
'''
Spyder (Python 3.6)
PyTorch 1.1.0
Windows 10 or Linux
...
def main():
    ...
    parser.add_argument('--weights',
                        default='../train/logs/FDNet0001/motiondeblur/SIDD/FDNet_/models/model_epoch_100.pth',
                        type=str, help='Path to weights')
    parser.add_argument('--gpus', default='0', type=str, help='CUDA_VISIBLE_DEVICES')
    parser.add_argument('--arch', default='FDNet', type=str, help='arch')
    ...
    from dataset.mydataset import loaddata,loadtest
    x, y = loadtest()
    for idx in range(len(x)):
        ...
        name = r'G:\ffp\ExperimentPaper\FDNet\Ex1\result\FDUNet' + '/' + str(idx) + '.tif'
        pp.save(name)
        ...
        # name = r'G:\ffp\result\denosing\tfd/Unet/log_test/label' + '/' + str(idx) + '.png'
        # pp1.save(name)
        # picture1 = (255 * img_net/(max1)).astype('uint8')
        # pp1 = Image.fromarray(picture1)
        # name = r'G:\ffp\result\denosing\tfd/Unet/log_test/net' + '/' + str(idx) + '.png'
        # pp1.save(name)

        # picture1 = (255 * img_org / (np.max(img_org))).astype('uint8')
        # pp1 = Image.fromarray(picture1)
        # name = r'G:\ffp\result\denosing\tfd/Unet/log_test/data' + '/' + str(idx) + '.png'
        # pp1.save(name)
        # img_net = tensor2uint(img_net)
        # img_H = tensor2uint(img_H)
        img_net = np.uint8((img_net/(max1) * 255.0).round())
        img_H = np.uint8((img_H/(max1) * 255.0).round())
        # img_net = np.uint8((img_net / (np.max(img_net)) * 255.0).round())
        # img_H = np.uint8((img_H / (max1) * 255.0).round())
    ...
    name2 = r'G:\ffp\ExperimentPaper\FDNet\Ex1\result' + '/' + 'FDUNet' + '.mat'
