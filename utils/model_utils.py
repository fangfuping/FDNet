...
def get_arch(opt):
    from model import Uformer, UNet, FDNet, FDUformer,DCNet
    ...
    if arch == 'UNet':
        ...
        model_restoration = FDNet(dim=opt.embed_dim)
    elif arch == 'DCNet':
        model_restoration = DCNet(dim=opt.embed_dim)
    elif arch == 'Uformer':
        ...
        model_restoration = FDUformer(img_size=opt.train_ps,embed_dim=opt.embed_dim,win_size=8,token_projection='linear',token_mlp='leff',modulator=True)
    elif arch == 'Uformer_T':
        ...
        model_restoration = Uformer(img_size=512, embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True,dd_in=opt.dd_in)
    elif arch == 'Uformer_1':
        depths = [2, 2, 2, 2, 2, 2, 2, 2, 2]
        model_restoration = Uformer(img_size=512, embed_dim=16, depths=depths,
